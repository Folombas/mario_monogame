...
namespace mario_monogame.Core.GameObjects
{
    ...
    public class AppleTree
    {
        ...
        private readonly List<TreeFoliage> _foliage;
        ...
        public AppleTree(GraphicsDevice graphicsDevice, Vector2 position, float scale = 1f)
        {
            ...
            // Создаём крону дерева (зелёная масса)
            _foliage = new List<TreeFoliage>
            {
                new TreeFoliage(position + new Vector2(0, -90) * scale, 50 * scale),
                new TreeFoliage(position + new Vector2(-40, -80) * scale, 35 * scale),
                new TreeFoliage(position + new Vector2(40, -80) * scale, 35 * scale),
                new TreeFoliage(position + new Vector2(-60, -100) * scale, 25 * scale),
                new TreeFoliage(position + new Vector2(60, -100) * scale, 25 * scale),
                new TreeFoliage(position + new Vector2(0, -120) * scale, 30 * scale),
            };
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            // Рисуем крону дерева (зелёная масса)
            foreach (var foliage in _foliage)
            {
                foliage.Draw(spriteBatch, _pixelTexture);
            }
    ...
    public class TreeFoliage
    {
        ...
        public TreeFoliage(Vector2 position, float radius)
        {
            _position = position;
            _radius = radius;
        }
        ...
        {
            DrawCircle(spriteBatch, _position, _radius, new Color(34, 139, 34), pixelTexture);
        }

        private void DrawCircle(SpriteBatch spriteBatch, Vector2 center, float radius, Color color, Texture2D pixelTexture)
        {
            int segments = 36;
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                DrawTriangle(spriteBatch, center, point1, point2, color, pixelTexture);
            }
        }

        private void DrawTriangle(SpriteBatch spriteBatch, Vector2 p1, Vector2 p2, Vector2 p3, Color color, Texture2D pixelTexture)
        {
